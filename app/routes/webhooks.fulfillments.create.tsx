...
import { activateReservedAuthItemsFromFulfillment } from "../models/auth-items.server";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    await activateReservedAuthItemsFromFulfillment({
      shopId: shopRecord.id,
      fulfillment: payload,
    });
    ...
    await markWebhookFailed(
      ...
      error instanceof Error
        ? error.message
        : "Unknown fulfillments/create error",
    );
    throw error;
  }
};
