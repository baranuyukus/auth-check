...
import { syncProductsFromWebhookPayload } from "../models/products.server";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    await syncProductsFromWebhookPayload(shopRecord.id, payload);
    await markWebhookProcessed(receipt.event.id);
    return new Response();
  } catch (error) {
    await markWebhookFailed(
      receipt.event.id,
      error instanceof Error ? error.message : "Unknown products/update error",
    );
    throw error;
  }
};
